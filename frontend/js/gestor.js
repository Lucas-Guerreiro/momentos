...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadLicenses() {
        tableBody.innerHTML = `<tr><td colspan="7" class="loading-td">Carregando licenças...</td></tr>`;
        try {
            allLicenses = await window.AuthService.listLicenses();
            updateKPIs();
            renderTable();
        } catch (error) {
            tableBody.innerHTML = `<tr><td colspan="7" style="color:#ef4444; text-align:center;">Erro ao carregar licenças.</td></tr>`;
        }
    }

    // 5. Atualizar KPIs
    function updateKPIs() {
        kpiTotal.textContent = allLicenses.length;
        kpiAtivas.textContent = allLicenses.filter(l => l.status === 'ativa').length;
        kpiAparelhos.textContent = allLicenses.filter(l => l.device_id).length;
        kpiDisponiveis.textContent = allLicenses.filter(l => l.status === 'disponivel').length;
    }
    ...
    function renderTable() {
        ...
        if (filtered.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="7" style="text-align:center; padding: 2rem; color: var(--text-muted);">Nenhuma licença encontrada.</td></tr>`;
            return;
        }
    ...
    formCreate.addEventListener('submit', async (e) => {
        ...
        const maxDev = inputMaxDevices.value;
        ...
        try {
            ...
            if (res.success && res.license) {
                ...
                newLicKeyDisplay.textContent = lic.chave;
                newLicPlanBadge.textContent = `${plano.toUpperCase()} • ${dias} DIAS`;
                newLicBox.style.display = 'block';

                btnCopyNewKey.onclick = () => copyText(lic.chave);
